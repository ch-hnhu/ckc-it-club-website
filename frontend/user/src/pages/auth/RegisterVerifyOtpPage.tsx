...
export default function RegisterVerifyOtpPage() {
	...
	const state = location.state as { email?: string; credentials?: RegisterCredentials } | null;
	...
	useEffect(() => {
		if (!email || !credentials) navigate("/register", { replace: true });
	}, [email, credentials, navigate]);
	...
	const handleResend = async () => {
		if (cooldown > 0 || resending || !credentials) return;
		setResending(true);
		setError(null);
		try {
			await sendRegistrationOtp(credentials);
			setDigits(Array(OTP_LENGTH).fill(""));
