...
const MyPointsPage: React.FC = () => {
	...
	useEffect(() => {
		...
		Promise.all([gamificationService.getMe(), gamificationService.getHistory(1, 20)])
	...
	if (!user) {
		return (
			<div className='mx-auto max-w-2xl px-4 py-16 text-center'>
				...
				<p className='mt-2 text-gray-600'>Vui lòng đăng nhập để xem điểm hoạt động của bạn.</p>
				<Link
					to='/login'
					className='mt-6 inline-block rounded-lg border-2 border-black bg-[var(--color-pastel-green)] px-6 py-2.5 font-bold text-black shadow-[3px_3px_0_#111] transition-transform hover:-translate-y-0.5'>
					Đăng nhập
				</Link>
			</div>
		);
		...
		<div className='mx-auto max-w-4xl px-4 py-8 md:py-10'>
			<div className='mb-8 flex items-center justify-between'>
				...
				<Link
					...
					className='hidden items-center gap-2 rounded-lg border-2 border-black bg-white px-4 py-2 font-bold text-black shadow-[3px_3px_0_#111] transition-transform hover:-translate-y-0.5 sm:flex'>
			...
			<div className='mb-8 grid gap-4 sm:grid-cols-3'>
				...
				/>
				<StatCard
					label='Hạng tuần / tổng'
					value={`#${me?.rank_weekly ?? "-"} / #${me?.rank_all_time ?? "-"}`}
