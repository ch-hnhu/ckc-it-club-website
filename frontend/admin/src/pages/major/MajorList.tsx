...
function MajorList() {
	...
	const handleDelete = async (major: Major) => {
		if (!window.confirm(`Bạn có chắc muốn xóa ngành "${major.label}"?`)) return;

		try {
			await majorService.deleteMajor(major.id);
			toast.success("Xóa ngành thành công.", { position: "top-right" });
			await fetchMajors();
		} catch (error) {
			console.error(error);
		...
		<div className='h-full flex-1 flex-col'>
			<div className='flex items-center p-4 md:p-6 lg:p-8'>
				<div className='flex flex-col gap-1'>
					<h2 className='text-2xl font-semibold tracking-tight'>Quản lý ngành</h2>
						...
						<TableHeader>
							<TableRow>
								...
								<TableHead>
									<Button
										variant='ghost'
										onClick={() => handleSort("school_classes_count")}
										className='-ml-4 h-8 hover:bg-muted-foreground/10'>
										Số lượng lớp
										{getSortIcon("school_classes_count")}
									</Button>
								</TableHead>
								<TableHead>
									<Button
						...
						<TableBody>
							{majors.map((major) => (
								<TableRow key={major.id}>
									<TableCell>
										<Checkbox
											aria-label={`Select major ${major.id}`}
											checked={isSelected(major.id)}
											onCheckedChange={(checked) =>
												toggleOne(major.id, checked === true)
											}
										/>
									</TableCell>
								...
								<TableCell colSpan={8}>
									<div className='flex items-center justify-between px-2'>
										<div className='flex-1 text-sm text-muted-foreground'>
											{majors.length} of {meta.total} row(s) displayed.
										</div>
