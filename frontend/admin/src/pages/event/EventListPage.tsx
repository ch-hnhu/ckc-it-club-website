...
function EventListPage() {
	...
	const handleChangeStatus = async (event: EventRecord, next: EventStatus) => {
		try {
			...
			setEvents((prev) =>
				prev.map((e) => (e.id === event.id ? { ...e, status: syncedStatus } : e)),
			);
	...
	const getNextActions = (status: EventStatus): NextAction[] => {
		if (status === "draft")
			return [{ next: "published", label: "Đăng sự kiện", icon: "publish" }];
		if (status === "published")
			return [{ next: "cancelled", label: "Hủy sự kiện", icon: "cancel" }];
		if (status === "ongoing")
			return [{ next: "cancelled", label: "Hủy sự kiện", icon: "cancel" }];
		if (status === "cancelled")
			return [{ next: "published", label: "Đăng lại sự kiện", icon: "publish" }];
		return [];
	};
			...
			<div className='space-y-6 p-4 md:p-6 lg:space-y-8 lg:p-8'>
				...
				<div className='space-y-2'>
					...
					<p className='text-muted-foreground'>
						Tạo, đăng và theo dõi toàn bộ sự kiện của CLB. Kiểm soát trạng thái từng sự
						kiện từ bản nháp đến khi kết thúc.
					</p>
				</div>
							...
							<TableHeader>
								<TableRow>
									...
									<TableHead className='w-[120px]'>
										<Button
											variant='ghost'
											onClick={() => handleSort("registrations_count")}
											className='-ml-4 h-8 hover:bg-muted-foreground/10'>
											...
											<TableCell>
												<div className='space-y-0.5 text-sm'>
													...
													<p className='text-xs text-muted-foreground'>
														Check-in:{" "}
														{event.check_ins_count.toLocaleString(
															"vi-VN",
														)}
													</p>
												</div>
											...
											<TableCell>
												<DropdownMenu>
													...
													<DropdownMenuContent
														...
														{getNextActions(event.status).map(
															({ next, label, icon }) => (
																<DropdownMenuItem
																	key={next}
																	onClick={() =>
									...
									<TableCell colSpan={10}>
										<div className='flex items-center justify-between px-2'>
											<p className='flex-1 text-sm text-muted-foreground'>
												Đang hiển thị {events.length} trên tổng {meta.total}{" "}
												sự kiện.
											</p>
					...
					{deleteTarget && (
						<>
							...
							<div className='space-y-2 text-sm text-muted-foreground'>
								...
								<p>
									Sự kiện sẽ không còn hiển thị với thành viên. Các lượt đăng ký
									và check-in liên quan cũng sẽ không truy cập được.
								</p>
							</div>
							<DialogFooter>
								<Button
