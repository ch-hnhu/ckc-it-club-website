...
function SchoolClassList() {
	...
	const fetchClasses = async () => {
		try {
			...
			toast.error("Không thể tải danh sách lớp.", { position: "top-right" });
		}
	};
	...
	const handleDelete = async (schoolClass: SchoolClass) => {
		if (!window.confirm(`Bạn có chắc muốn xóa lớp "${schoolClass.label}"?`)) return;

		try {
			await schoolClassService.deleteSchoolClass(schoolClass.id);
		...
		<div className='h-full flex-1 flex-col'>
			<div className='flex items-center p-4 md:p-6 lg:p-8'>
				<div className='flex flex-col gap-1'>
					<h2 className='text-2xl font-semibold tracking-tight'>Quản lý lớp</h2>
						...
						<TableBody>
							{classes.map((schoolClass) => (
								<TableRow key={schoolClass.id}>
									<TableCell>
										<Checkbox
											aria-label={`Select class ${schoolClass.id}`}
											checked={isSelected(schoolClass.id)}
											onCheckedChange={(checked) =>
												toggleOne(schoolClass.id, checked === true)
											}
										/>
									</TableCell>
								...
								<TableCell colSpan={8}>
									<div className='flex items-center justify-between px-2'>
										<div className='flex-1 text-sm text-muted-foreground'>
											{classes.length} of {meta.total} row(s) displayed.
										</div>
			...
			<SchoolClassFormModal
				...
				onOpenChange={(open) => {
					setIsFormOpen(open);
					if (!open) setSelectedSchoolClass(null);
				}}
				schoolClass={selectedSchoolClass}
				onSuccess={() => void fetchClasses()}
			/>
		</div>
	);
}
