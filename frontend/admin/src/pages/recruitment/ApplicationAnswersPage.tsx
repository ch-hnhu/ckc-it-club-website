...
function getAnswerPreview(application: ClubApplicationRecord) {
	...
	return (
		firstMeaningfulAnswer?.answer_value || "Ứng viên chưa có câu trả lời nổi bật để xem nhanh."
	);
}
			...
			<div className='space-y-6 p-4 md:p-6 lg:space-y-8 lg:p-8'>
					...
					<div className='grid gap-8 px-6 py-7 md:px-8 md:py-9 xl:grid-cols-[1.2fr_0.8fr] xl:items-end'>
						<div className='space-y-4'>
							...
							<div className='space-y-3'>
								<h1 className='max-w-4xl text-[2rem] font-semibold leading-tight md:text-[3.5rem] md:leading-[1.02]'>
									Duyệt câu trả lời theo từng ứng viên, không cần mở từng hồ sơ
									riêng lẻ
								</h1>
								<p className='max-w-3xl text-sm leading-7 text-primary-foreground/72 md:text-base'>
									Tập trung vào chất lượng câu trả lời, lọc nhanh theo trạng thái
									và câu hỏi, sau đó chuyển sang trang chi tiết hồ sơ khi cần xử
									lý sâu hơn.
								</p>
							</div>
						</div>
						...
						<div className='grid gap-3 sm:grid-cols-3 xl:grid-cols-1'>
							<div className='rounded-[24px] border border-primary-foreground/10 bg-primary-foreground/8 p-4 backdrop-blur-sm'>
								<p className='text-xs font-semibold uppercase tracking-[0.22em] text-primary-foreground/55'>
									Hồ sơ có câu trả lời
								</p>
								<p className='mt-2 text-3xl font-semibold'>{applications.length}</p>
								<p className='mt-2 text-sm text-primary-foreground/65'>
									Tổng hồ sơ đang đồng bộ vào giao diện.
								</p>
							</div>
							<div className='rounded-[24px] border border-primary-foreground/10 bg-primary-foreground/8 p-4 backdrop-blur-sm'>
								<p className='text-xs font-semibold uppercase tracking-[0.22em] text-primary-foreground/55'>
								...
								<p className='mt-2 text-sm text-primary-foreground/65'>
									Bao gồm các câu text, textarea và lựa chọn.
								</p>
							</div>
							<div className='rounded-[24px] border border-primary-foreground/10 bg-primary-foreground/8 p-4 backdrop-blur-sm'>
								<p className='text-xs font-semibold uppercase tracking-[0.22em] text-primary-foreground/55'>
								...
								<p className='mt-2 text-sm text-primary-foreground/65'>
									Các hồ sơ đã qua bước chờ duyệt ban đầu.
								</p>
							</div>
						</div>
					</div>
				...
				<section className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
					...
					<MetricCard
						...
						value={statusFilter === "all" ? "01" : "02"}
						description='Tự động tăng khi bạn lọc theo trạng thái hoặc câu hỏi.'
					...
					<Card className='overflow-hidden rounded-[28px] border-border bg-card shadow-sm'>
						<CardHeader className='space-y-5 border-b border-border bg-muted/20 px-5 py-5 md:px-6 md:py-6'>
							<div className='flex flex-wrap items-start justify-between gap-3'>
								...
								<div className='bg-background text-primary rounded-full border border-border px-3 py-1.5 text-sm font-medium'>
							...
							<div className='grid gap-3'>
								<div className='relative'>
									...
									<Input
										...
										className='bg-background h-11 rounded-2xl border-border pl-10'
									/>
								</div>
						...
						<CardContent className='p-4 md:p-5'>
							{loading ? (
									...
									{filteredApplications.map((application) => {
										...
										return (
											<button
												...
												className={`w-full rounded-[24px] border p-4 text-left transition ${isActive ? "border-primary/30 bg-primary/5 shadow-sm" : "border-border bg-card hover:bg-muted/20"}`}>
												<div className='flex items-start justify-between gap-3'>
													<div className='flex min-w-0 items-start gap-3'>
														<div className='bg-primary text-primary-foreground flex size-12 shrink-0 items-center justify-center rounded-2xl text-sm font-semibold'>
													...
													<Badge
														variant='outline'
														className='bg-background text-primary rounded-full border-border'>
								...
								<div className='bg-muted/30 rounded-[24px] border border-dashed border-border px-6 py-12 text-center'>
									...
									<p className='text-muted-foreground mx-auto mt-2 max-w-md text-sm leading-7'>
										Hãy nới điều kiện lọc hoặc thay đổi từ khóa tìm kiếm để tiếp
										tục duyệt câu trả lời.
									</p>
								</div>
							)}
						</CardContent>
						...
						{loading ? (
										...
										<div className='space-y-3'>
											<div className='flex flex-wrap items-center gap-2'>
												...
												<Badge
													variant='outline'
													className={
														getStatusConfig(selectedApplication.status)
															.className
													}>
											...
											<div>
												<CardTitle className='text-foreground text-[1.9rem] font-semibold leading-tight'>
													{getApplicantName(selectedApplication)}
												</CardTitle>
												<CardDescription className='text-muted-foreground mt-2 max-w-2xl text-sm leading-7'>
													Xem toàn bộ câu trả lời đã lưu cho ứng viên này
													và chuyển sang trang chi tiết hồ sơ nếu cần xử
													lý trạng thái hoặc ghi chú.
												</CardDescription>
											</div>
										</div>
								...
								<CardContent className='space-y-6 p-5 md:p-6'>
									<div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
										<div className='bg-muted/40 rounded-[22px] border border-border p-4'>
											...
											<p className='text-foreground mt-3 text-base font-semibold leading-7'>
												{getApplicantName(selectedApplication)}
											</p>
										</div>
										<div className='bg-muted/40 rounded-[22px] border border-border p-4'>
											...
											<p className='text-foreground mt-3 break-words text-base font-semibold leading-7'>
											...
											{selectedApplication.answers.length > 0 ? (
													...
													<div
														...
														<div className='flex flex-wrap items-start justify-between gap-3'>
															<div className='space-y-2'>
																...
																<h3 className='text-foreground text-lg font-semibold leading-8'>
																	{answer.question_label}
																</h3>
															</div>
															<Badge
																variant='outline'
																className='bg-muted/30 text-primary rounded-full border-border'>
														...
														<div className='bg-muted/40 mt-4 rounded-[20px] border border-border px-4 py-4'>
															...
															<p className='text-foreground mt-2 break-words text-base leading-8'>
																{answer.answer_value ||
												...
												<div className='bg-background rounded-[24px] border border-dashed border-border px-6 py-12 text-center'>
													...
													<p className='text-muted-foreground mt-2 text-sm leading-7'>
														Hồ sơ này chưa ghi nhận dữ liệu trong danh
														sách câu trả lời.
													</p>
												</div>
											)}
							...
							<div className='flex min-h-[32rem] items-center justify-center p-6'>
								<div className='bg-muted/30 max-w-md rounded-[24px] border border-dashed border-border px-6 py-12 text-center'>
									...
									<p className='text-muted-foreground mt-2 text-sm leading-7'>
										Khi bạn chọn một ứng viên ở cột bên trái, toàn bộ nội dung
										trả lời sẽ xuất hiện tại đây.
									</p>
								</div>
							</div>
						)}
