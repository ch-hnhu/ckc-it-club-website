...
function CourseCategoryListPage() {
	...
	const { allSelected, isSelected, toggleAll, toggleOne } = useTableSelection(categories.map((c) => c.id));
	...
	useEffect(() => {
		...
		courseCategoryService.getCategories({
		...
		<div className="min-h-full bg-background">
			<div className="space-y-6 p-4 md:p-6 lg:space-y-8 lg:p-8">
				...
				<div className="grid gap-4 sm:grid-cols-3">
					<div className="rounded-2xl border border-teal-500/15 bg-teal-500/5 p-5 shadow-sm">
						...
						<p className="mt-1 text-xs text-muted-foreground">Danh mục đang tồn tại trong hệ thống.</p>
					</div>
					<div className="rounded-2xl border border-violet-500/15 bg-violet-500/5 p-5 shadow-sm">
						...
						<p className="mt-1 text-xs text-muted-foreground">Tổng số lần được gán cho khóa học.</p>
					</div>
					<div className="rounded-2xl border border-amber-500/15 bg-amber-500/5 p-5 shadow-sm">
						...
						<p className="mt-1 text-xs text-muted-foreground">Danh mục chưa gán cho khóa học nào.</p>
					</div>
				</div>
						...
						<Table>
							<TableHeader>
								<TableRow>
									...
									<TableHead className="w-[80px] px-4 text-muted-foreground">STT</TableHead>
									<TableHead>
									...
									<TableHead className="w-[120px]">
										<Button variant="ghost" onClick={() => handleSort("courses_count")} className="-ml-4 h-8 hover:bg-muted-foreground/10">
										...
										<TableRow key={category.id}>
											<TableCell>
												<Checkbox checked={isSelected(category.id)}
													onCheckedChange={(c) => toggleOne(category.id, c === true)} />
											</TableCell>
											...
											<TableCell className="text-sm text-muted-foreground">{category.courses_count}</TableCell>
									...
									<TableCell colSpan={7}>
										<div className="flex items-center justify-between px-2">
											<p className="flex-1 text-sm text-muted-foreground">
												Đang hiển thị {categories.length} trên tổng {meta.total} danh mục.
											</p>
			...
			<Dialog open={formOpen} onOpenChange={(o) => !o && setFormOpen(false)}>
				<DialogContent className="sm:max-w-[460px]">
					<DialogHeader>
						<DialogTitle>{editTarget ? "Sửa danh mục" : "Thêm danh mục mới"}</DialogTitle>
					</DialogHeader>
					...
					<DialogFooter>
						...
						<Button onClick={handleSave} disabled={isSaving}>
							{isSaving ? "Đang lưu..." : editTarget ? "Lưu thay đổi" : "Tạo danh mục"}
						</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
			...
			<Dialog open={Boolean(deleteTarget)} onOpenChange={(o) => !o && setDeleteTarget(null)}>
				<DialogContent className="sm:max-w-[440px]">
					{deleteTarget && (
						<>
							<DialogHeader><DialogTitle>Xác nhận xóa danh mục</DialogTitle></DialogHeader>
							<div className="space-y-3 text-sm text-muted-foreground">
								...
								{deleteTarget.courses_count > 0 && (
									<p className="rounded-md border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-amber-700">
										Danh mục này đang được dùng trong {deleteTarget.courses_count} khóa học.
										Xóa danh mục sẽ gỡ nó khỏi tất cả khóa học liên quan.
									</p>
								)}
