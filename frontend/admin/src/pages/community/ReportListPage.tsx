...
const NEXT_STATUS: Record<PostReportRecord["status"], { value: PostReportRecord["status"]; label: string }[]> = {
	pending: [
...
function StatusBadge({ status }: { status: PostReportRecord["status"] }) {
...
export default function ReportListPage() {
	...
	// Load stats
	useEffect(() => {
		reportService.getStats().then((r) => setStats(r.data)).catch(() => {});
	}, [reports]); // refresh after mutations
	...
	useEffect(() => {
		...
		reportService
			...
			.then((res) => {
				if (cancelled) return;
				setReports(res.data as unknown as PostReportRecord[]);
	...
	const handleUpdateStatus = async (report: PostReportRecord, status: PostReportRecord["status"]) => {
		try {
			const res = await reportService.updateStatus(report.id, status);
	...
	const handleHidePost = async (report: PostReportRecord) => {
		try {
			const res = await reportService.hidePost(report.id);
			setReports((prev) => prev.map((r) => r.id === report.id ? res.data : r));
			toast.success("Đã ẩn bài viết và đánh dấu đã xử lý.");
		} catch {
										...
										<TableCell>
											{report.post ? (
												<div className="flex items-start gap-1.5">
													<a
														href={`http://localhost:5174/cong-dong/bai-viet/${report.post.id}`}
														target="_blank"
												...
												<span className="text-xs italic text-muted-foreground/50">Bài viết đã xóa</span>
											)}
										...
										<TableCell>
											<DropdownMenu>
												...
												<DropdownMenuContent align="end" className="w-52">
													...
													{report.post && report.post.status !== "hidden" && (
														<DropdownMenuItem
