...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
	return (
		<thead
			...
			className={cn("bg-muted/10 [&_tr]:border-b", className)}
			{...props}
		/>
	);
}
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
	return (
		<th
			...
			className={cn(
				"text-foreground h-11 px-4 text-left align-middle text-xs font-semibold tracking-[0.02em] whitespace-nowrap [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]",
				className,
			)}
			{...props}
		/>
