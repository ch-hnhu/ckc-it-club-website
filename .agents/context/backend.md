...
- `courses`
- learning courses include `audience` (`club_member`, `cao_thang_student`, `public`) to define who can learn the course. `public` still requires an authenticated account. Existing/new unspecified courses default to `cao_thang_student`.
...
- admin can create, update, set a non-banner value as the default, and delete nested values through `/club-informations/{clubInformation}/values`; value payloads support long-text `value`, `link`, `alt`, integer/null `position`, and `is_active`; HTML and Markdown values are not capped at 1000 characters.
...
- `2026-07-02`: Certificate rendering now preloads embedded Be Vietnam Pro/Roboto with the actual scene text plus a Vietnamese probe string before Konva draws. This is required because `cert-fonts.css` uses `unicode-range` subsets; loading only `document.fonts.load('400 40px "..."')` could load the latin subset but leave the Vietnamese subset pending, causing occasional misplaced diacritics in the rasterized PDF canvas. Existing PDFs are already raster images and must be reissued/regenerated to pick up the fix.
- `2026-07-01`: Certificate templates now use a single source of truth: `certificate_templates.design`. The legacy `html_content` field, model fillable/render helper, duplicate-copy path, and DomPDF fallback were removed; a new migration drops the old column for existing databases. `CertificateTemplateSeeder` seeds the current drag-drop default certificate through `design`, updates the existing default template (or `Mẫu mặc định`), keeps it as the single default, and copies tracked logo assets from `database/seeders/assets/certificate-templates/` to the public disk before saving. Designs store stable `/storage/...` paths; `CertificateTemplateController` resolves those paths to backend absolute URLs for the admin editor and normalizes them back to `/storage/...` on save.
...
- `2026-06-25`: User learning video/progress and quiz show/submit endpoints now reject lessons whose `session_start` is still in the future, while lesson detail remains readable so the frontend can show the "buổi học chưa diễn ra" panel.
- `2026-06-25`: User learning course detail now includes `max_offline_slots` so the frontend can distinguish parallel offline courses from online-only courses and apply the correct lesson unlock flow.
- `2026-06-25`: `CourseController` `store` and `update` now also nullify `enrollment_start`, `enrollment_deadline`, and `course_end` when `has_offline = false`, ensuring online-only courses never retain stale offline date columns.
...
- `2026-06-24`: Certificate templates became a **drag-drop canvas designer** (Canva-style). `certificate_templates.design` stores the scene `{canvas:{width,height,background:{color,image}}, elements:[...]}`. `CertificateTemplateController` now has full CRUD: `show`, `store`, `update`, `destroy` (blocked when `is_default`), `setDefault`, `duplicate`, `uploadAsset` (`POST .../assets` → stores to `certificate-assets` public disk), `preview` (`POST .../preview` → renders sample PDF as base64 data URI). **Important**: `store`/`update` persist `$request->input('design')` (NOT the validated array — `validate()` strips keys without rules like `canvas.background`). New render engine `App\Services\CertificateRenderer` uses **spatie/browsershot** (Chrome headless) to rebuild the Konva scene with real data → PDF (WYSIWYG with the editor); placeholders `{{name}} {{course}} {{issued_at}} {{cert_code}}`, and `qr`-type elements get a QR (endroid/qr-code) pointing at `{APP_URL}/verify/{cert_code}`. `CourseCertificateService::generate()` renders only via `CertificateRenderer` and rejects templates without `design.canvas`. Browsershot uses `base_path('node_modules')` (puppeteer installed there for dev); set `BROWSERSHOT_CHROME_PATH` env for prod (Dockerfile has a commented apk block for Chromium). Konva is vendored at `backend/public/vendor/konva.min.js`. The `/verify/{cert_code}` public page itself is NOT built yet (QR encodes the URL for the future page).
