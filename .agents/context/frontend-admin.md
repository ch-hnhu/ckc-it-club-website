...
- Learning course rows/forms include `audience` (`club_member`, `cao_thang_student`, `public`) with labels "Thành viên CLB", "Sinh viên Cao Thắng", and "Công khai"; `public` means any authenticated account can learn.
...
- `2026-06-26`: Course create/edit now keeps both "Trình độ" and "Đối tượng học" inside the right-side "Cấu hình khoá học" panel; the main information card is limited to course name and description.
- `2026-06-26`: Course edit/create no longer includes the "Số buổi học dự kiến" field because the backend course contract no longer has `total_lessons`. The "Đối tượng học" select now lives in the right-side "Cấu hình khoá học" panel with quiz threshold and certificate template settings.
- `2026-06-25`: Certificate editor got **free zoom/pan + a Layers panel**. The Konva Stage now fills its container (measured via `ResizeObserver`) and uses `zoom`/`stagePos` state instead of a fixed `SCALE`: scroll = pan, Ctrl/Cmd+scroll = zoom-to-cursor, plus a bottom-center overlay (− / % / + / fit) and auto fit-to-screen on open; the page is a shadowed rect floating in a gray area. Guide lines + transformer anchors scale by `1/zoom`. New right-sidebar **Layers** panel (`LayersPanel`) lists every element (front-most first) with select + per-row reorder (to-top/up/down/to-bottom via `moveLayer`/`moveLayerToEnd`) + delete; element array order = z-order (already persisted in `design`). Zoom/pan are view-only (not saved), so the PDF renderer is unaffected.
- `2026-06-25`: Course management now supports selecting and displaying the course learning audience. `CourseFormPage` submits `audience`, `CourseListPage` shows/sorts/filters the audience column, and `CourseDetailPage` displays it in badges/config.
